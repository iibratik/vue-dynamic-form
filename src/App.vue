  ...
  <form class="form-content" action="#">
    <div class="form-inputs">
      <div class="inputs" v-for="(input, index) in params.inputs" :key="index.id">
        <input  :type="input.type" v-if="input.type === 'text'" :placeholder="input.placeholder" :id="'input-' + input.id">
        <input class="form-control" :type="input.type"  v-if="input.type === 'email'" :placeholder="input.placeholder" :id="'input-' + input.id">
        <div :type="input.type" v-if="input.type === 'radio'">
        ...
        <div class="select-content" :style="{gridTemplateColumns: `repeat(${input['selector-per-block']}, 100px)` }" v-if="input.type === 'select'">
        <select >
...
import myParams from '../params.json';
export default {
  data() {
    return {
      date: null,
